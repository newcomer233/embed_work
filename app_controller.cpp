...
AppController::AppController(MAX7219& display)
    : mode("time"),
      ...
      max7219(display)
{
    startTimeMode(); // Time mode by default
}
...
void AppController::handleCommand(const std::string& input) {
    if (awaitingTimerInput) {
        if (input == "quit") {
            ...
            std::cout << "Temperature mode, input like s12 / r-5\n";
            ...
            if (totalSeconds <= 0) {
                std::cout << "Time must be greater than 0\n";
                return;
            }

            awaitingTimerInput = false;
            startTimerCountdown(totalSeconds);
            std::cout << "Timer started, input p to pause or resume.\n";
        } else {
            std::cout << "Format error, should be 4 digits (MMSS)\n";
        }
        return;
    }
    ...
    if (input == "time") {
        ...
        waitingWeatherInput = true;
        std::cout << "Enter weather and temperature (e.g. s12 / r-5)\n";
...
void AppController::startTimeMode() {
    ...
    std::cout << "Switched to time mode\n";
}
